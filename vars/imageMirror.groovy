...
def call(ImageMirrorInput input) {
    ...
    assert input.sourceImageVersion?.trim() : "Param sourceImageVersion should be defined."
    ...
    echo "sourceApi == $sourceApi"
    echo "destinationApi == $destinationApi"
    ...
    withDockerRegistry([credentialsId: "${input.sourceSecret}", url: "${input.sourceRegistry}"]) {
        withDockerRegistry([credentialsId: "${input.destinationSecret}", url: "${input.destinationRegistry}"]) {
            ...
            sh "oc image mirror --insecure=${input.insecure} ${source} ${destination}"
        }
    }
}
